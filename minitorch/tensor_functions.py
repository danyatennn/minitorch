...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        one = sig._ensure_tensor(1.0)
        neg_sig = sig.f.neg_map(sig)
        one_minus_sig = sig.f.add_zip(one, neg_sig)
        local_grad = sig.f.mul_zip(sig, one_minus_sig)
        return sig.f.mul_zip(local_grad, grad_output)
